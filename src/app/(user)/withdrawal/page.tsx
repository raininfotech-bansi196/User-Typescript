"use client"
import TableLoader from '@/componanats/TableLoader';
import AddWithdrawal from '@/componanats/withdrawal/AddWithdrawal';
...
const page = () => {
    ...
    const [withdrawalList, setWithdrawalList] = React.useState<WithdrawalListType[]>([])
    ...
    const getWithdrawalList = async () => {
        try {
            setSpinner(false);
            const response = await fetchApi(`/get-withdrawal-request`, JSON.stringify({ page: page, address: address, orderColumn: orderClm, order: order, chain, coin }), "POST");
    ...
    const getWalletListFilter = async () => {
        const response = await fetchApi(`/filter/withdrawal-list-filter`, "", "GET");
        if (response?.statusCode === 200) {
        ...
        <>
            <section className="page-info-section">
                <div className="container">
                    <h2 className='fw-bold fw-bolder'>Withdrawal</h2>
                    <div className="site-beradcamb">
                        <Link href="/">Home</Link>
                        <span>
                            <i className="fa fa-angle-right" /> Withdrawal
                        </span>
                    </div>
                </div>
            </section>
            <section className='container py-5'>
                <div className='text-right mb-4'>
                    <button onClick={() => setShow(true)} className='site-btn sb-gradients sbg-line ml-auto d-inline-block'>Add Withdrawal</button>
                </div>
                <div className='profile-details p-4'>
                        ...
                        <table className="table m-0 table-bordered table-hover">
                            <thead>
                                <tr className='border-none'>
                                    ...
                                    <th className='sort' onClick={() => sortData(6, order === 0 ? 1 : 0)}>
                                        Created On
                                        <span className='position-relative d-inline-flex g-1 flex-column'>
                                            <i className={`fa-solid fa-sort-up ${orderClm === 6 && order === 0 ? 'sort-enable' : 'sort-disable'}`}></i>
                                            <i className={`fa-solid fa-sort-down position-absolute ${orderClm === 6 && order === 1 ? 'sort-enable' : 'sort-disable'}`}></i>
                                        </span>
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                {
                                    !spinner && withdrawalList?.map((x, index) => {
                                        return (
                                            <tr key={index}>
                                                <td>{index + 1}</td>
                                                <td>{x?.chainName}</td>
                                                ...
                                                <td>{x?.amount}</td>
                                                <td>
                                                    <span className={`badge badge-${x?.status === 0 ? "warning" : x?.status === 1 ? "success" : "danger"}`}>{x?.status === 0 ? "Pending" : x?.status === 1 ? "Confirmed" : "Rejected"}</span>
                                                    {/* {x?.status} */}
                                                </td>
                                                <td>
                                                    <div className='d-flex w-100 align-items-center justify-content-between'>
                                                        <Link target='_blank' className='address-link' href={`https://testnet.bscscan.com/address/${x?.toAddress}`}>{shortenAddress(x?.toAddress)}</Link>
                                                        <span className='address-copy' onClick={() => copyAddress(x?.toAddress)}><i className='fa fa-copy'></i></span>
                                                    </div>
                                                </td>
                                                <td>
                                                    <div className='d-flex w-100 align-items-center justify-content-between'>
                                                        <Link target='_blank' className='address-link' href={`https://testnet.bscscan.com/address/${x?.fromAddress}`}>{shortenAddress(x?.fromAddress)}</Link>
                                                        <span className='address-copy' onClick={() => copyAddress(x?.fromAddress)}><i className='fa fa-copy'></i></span>
                                                    </div>
                                                </td>
                                ...
                                {
                                    spinner || withdrawalList?.length === 0 ?
                                        <tr className='table-loader'>
                                            <td colSpan={9}>
                                                <div className='text-center'>
                                                    {spinner ? <TableLoader /> : withdrawalList?.length === 0 &&
                    ...
                    {
                        withdrawalList.length > 0 ? <div className="row w-100 paginationBox justify-content-end">
                            <ReactPaginate
                                className=''
                                breakLabel={'...'}
            ...
            <AddWithdrawal isShow={isShow} handleClose={handleClose} />
        </>
    )
}
